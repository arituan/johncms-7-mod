...
echo '<!DOCTYPE html>' .
    ...
    "\n" . '<meta name="description" content="' . (isset($meta) ? $meta['desc'] : $config->meta_desc) . '">';
if (isset($meta))
    echo "\n" . '<meta property="og:site_name" content="' . htmlspecialchars($_SERVER['HTTP_HOST']) . '" />' .
            "\n" . '<meta property="og:type" content="article" />' .
            "\n" . '<meta property="og:url" content="' . $meta['link'] . '" />' .
            "\n" . '<meta property="og:title" content="' . $textl . '" />' .
            "\n" . '<meta property="og:description" content="' .$meta['desc'] . '" />' .
            "\n" . '<meta property="twitter:card" content="summary" />' .
            "\n" . '<meta property="twitter:title" content="' . $textl . '" />' .
...
echo '<table style="width: 100%;" class="header"><tr>' .
    '<td>' . _t('Hi', 'system') . ' ' . ($systemUser->id ? '<b>' . $tools->displayUsername($systemUser->name, $systemUser->rights) . '</b>' : _t('Guest', 'system')) . '</td>'.
     ($headmod == 'mainpage' && count($config->lng_list) > 1 ? '<td align="right"><a href="' . $config->homeurl . '/go.php?lng"><b>' . strtoupper($locale) . '</b></a>&#160;<a href="' . $config->homeurl . '/go.php?lng">' . $tools->getFlag($locale) . '</a></td>' : '') .
    '</tr></table>';

// Главное меню пользователя
echo '<div class="tmn">' .
    (isset($_GET['err']) || $headmod != 'mainpage' || ($headmod == 'mainpage' && $act) ? '<a href="' . $config['homeurl'] . '" class="pagenav">' . $tools->image('menu_home.png') . '</a> ' : '') .
    ($systemUser->id && $headmod != 'office' ? '<a href="' . $config['homeurl'] . '/profile/?act=office" class="pagenav">' . $tools->image('menu_cabinet.png') . _t('Personal', 'system') . '</a> ' : '') .
    (!$systemUser->id && $headmod != 'login' ? '<a href="' . $config['homeurl'] . '/login.php" class="pagenav">' . _t('Login', 'system') . '</a> ' : '') .
    (!$systemUser->id && $headmod != 'registration' ? '<a href="' . $config['homeurl'] . '/registration" class="pagenav">' . _t('Registration', 'system') . '</a> ' : '') .
