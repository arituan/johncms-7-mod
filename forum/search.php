    ...
    default:
        ...
        if ($search && !$error) {
            ...
            if ($total) {
                $to_history = true;
                $req = $db->query("
                    SELECT `forum`.*, MATCH (`text`) AGAINST ($query IN BOOLEAN MODE) as `rel`, `users`.`rights`
                    FROM `forum` INNER JOIN `users` ON `users`.`id` = `forum`.`user_id`
                    WHERE MATCH (`text`) AGAINST ($query IN BOOLEAN MODE)
                ...
                while ($res = $req->fetch()) {
                    ...
                    echo '<br /><a href="' . ($search_t ? $tools->rewriteUrl($res['text'], $res['id']) : $tools->rewriteUrl($res_t['text'], $res_t['id'])) . '">' . _t('Go to Topic') . '</a>' . ($search_t ? ''
