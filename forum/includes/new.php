...
if ($systemUser->isValid()) {
    switch ($do) {
        ...
        case 'period':
            ...
            if ($count) {
                if ($systemUser->rights == 9) {
                    $req = $db->query("SELECT `forum`.*, `users`.`rights` FROM `forum` INNER JOIN `users` ON `forum`.`user_id` = `users`.`id` WHERE `type`='t' AND `time` > '" . $vr1 . "' ORDER BY `time` DESC LIMIT " . $start . "," . $kmess);
                } else {
                    $req = $db->query("SELECT `forum`.*, `users`.`rights` FROM `forum` INNER JOIN `users` ON `forum`.`user_id` = `users`.`id` WHERE `type`='t' AND `time` > '" . $vr1 . "' AND `close` != '1' ORDER BY `time` DESC LIMIT " . $start . "," . $kmess);
                }
                ...
                for ($i = 0; $res = $req->fetch(); ++$i) {
                    ...
                    $colmes = $db->query("SELECT `forum`.`user_id`, `forum`.`from`, `forum`.`time`, `users`.`rights` FROM `forum` INNER JOIN `users` ON `forum`.`user_id` = `users`.`id` WHERE `refid` = '" . $res['id'] . "' AND `type` = 'm'" . ($systemUser->rights >= 7 ? '' : " AND `close` != '1'") . " ORDER BY `time` DESC");
                    ...
                    echo '<br /><div class="sub"><a href="' . $tools->rewriteUrl($razd['text'], $razd['id']) . '">' . $frm['text'] . '&#160;/&#160;' . $razd['text'] . '</a><br />';
            ...
            if ($total > 0) {
                ...
                for ($i = 0; $res = $req->fetch(); ++$i) {
                    ...
                    echo '<div class="sub">' . $res['from'] . ($colmes1 > 1 ? '&#160;/&#160;' . $nick['from'] : '') .
                        ...
                        '<a href="' . $tools->rewriteUrl($frm['text'], $frm['id']) . '">' . $frm['text'] . '</a>&#160;/&#160;<a href="' . $tools->rewriteUrl($razd['text'], $razd['id']) . '">' . $razd['text'] . '</a>' .
                        '</div></div>';
                }
            } else {
    ...
    $req = $db->query("SELECT `forum`.*, `users`.`rights` FROM `forum` INNER JOIN `users` ON `forum`.`user_id` = `users`.`id` WHERE `type` = 't' AND `close` != '1' ORDER BY `time` DESC LIMIT 10");

    if ($req->rowCount()) {
        for ($i = 0; $res = $req->fetch(); ++$i) {
            $razd = $db->query("SELECT `id`, `refid`, `text` FROM `forum` WHERE type='r' AND id='" . $res['refid'] . "' LIMIT 1")->fetch();
            $frm = $db->query("SELECT `id`, `refid`, `text` FROM `forum` WHERE type='f' AND id='" . $razd['refid'] . "' LIMIT 1")->fetch();
            $nikuser = $db->query("SELECT `from`, `time`, `rights` FROM `forum` INNER JOIN `users` ON `forum`.`user_id` = `users`.`id` WHERE `type` = 'm' AND `close` != '1' AND `refid` = '" . $res['id'] . "'ORDER BY `time` DESC");
            ...
            echo implode('', array_filter($icons));
            echo '<a href="'. $tools->rewriteUrl($res['text'], $res['id']) . '">' . (empty($res['text']) ? '-----' : $res['text']) . '</a>&#160;[' . $colmes1 . ']';
