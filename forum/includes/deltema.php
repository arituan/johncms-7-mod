...
if ($systemUser->rights == 3 || $systemUser->rights >= 6) {
    ...
    if (isset($_POST['submit'])) {
        ...
        if ($del == 2 && $systemUser->rights == 9) {
            ...
            $req_m = $db->query("SELECT `id` FROM `forum` WHERE `type` = 'm' AND `refid` = '$id'");
            if ($req_m->rowCount()) {
                $list = [];
                while ($res_m = $req_m->fetch()) {
                    $list[] = $res_m['id'];
                }
                $in = implode(', ', $list);
                $db->exec("DELETE FROM `cms_forum_thank` WHERE `fid` IN (" . $in . ")");
                $db->exec("DELETE FROM `cms_mail` WHERE `sys` = '1' AND `sys_ext` IN (" . $in . ")");
            }
