...
$th = filter_has_var(INPUT_POST, 'th')
    ? $tools->substring(filter_input(INPUT_POST, 'th', FILTER_SANITIZE_SPECIAL_CHARS, ['flag' => FILTER_FLAG_ENCODE_HIGH]), 100)
    : '';
$th = $container->get('bbcode')->notags($th); //remove all bbcode in topic name
...
if (isset($_POST['submit'])
    ...
    if (!$error) {
        ...
        $db->exec("INSERT INTO `cms_forum_rdm` SET
            ...
            `time`='" . time() . "'
        ");
        $link_topic = $tools->rewriteUrl($th, $rid);
        ...
        $msg_no = '[url=' . $config->homeurl . '/profile/?user=' . $systemUser->id . ']' . $tools->displayUsername($systemUser->name, $systemUser->rights, true) . '[/url] ' . _t('has mentions you in a topic') . ': [url=' . $link_topic . ']' . strip_tags($th) . '[/url]';
        for ($i=0; $i < $max_users; $i++) {
            $username = strip_tags(trim($list[$i]));
            if ($username == $systemUser->name) continue;
            $req = $db->query("SELECT `id` FROM `users` WHERE `name_lat` = " . $db->quote($username) . " LIMIT 1;");
    ...
    echo '<textarea rows="' . $systemUser->getConfig()->fieldHeight . '" name="msg" onkeyup="auto_grow(this)">' . (isset($_POST['msg']) ? $tools->checkout($_POST['msg']) : '') . '</textarea></p>' .
