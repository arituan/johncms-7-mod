...
$req = $db->query("SELECT `refid`, `user_id`, `close`, `time`, `kedit`, `text` FROM `forum` WHERE `id` = '$id' AND `type` = 'm' " . ($systemUser->rights >= 7 ? "" : " AND `close` != '1'"));

if ($req->rowCount()) {
    // Предварительные проверки
    $res = $req->fetch();

    $topic = $db->query("SELECT `refid`, `curators`, `text` FROM `forum` WHERE `id` = " . $res['refid'])->fetch();
    ...
    $link = $tools->rewriteUrl($topic['text'], $res['refid'], $page);
    $error = false;

    if ($systemUser->rights == 3 || $systemUser->rights >= 6) {
        // Проверка для Администрации
        if ($res['user_id'] != $systemUser->id) {
            $req_u = $db->query("SELECT `rights` FROM `users` WHERE `id` = '" . $res['user_id'] . "'");
        ...
        if (!$error) {
            $section = $db->query("SELECT `edit` FROM `forum` WHERE `id` = " . $topic['refid'])->fetch();
            $allow = !empty($section['edit']) ? intval($section['edit']) : 0;
            $check = true;

            if ($allow == 2) {
                $first = $db->query("SELECT `user_id`, `id` FROM `forum` WHERE `refid` = '" . $res['refid'] . "' ORDER BY `id` ASC LIMIT 1")->fetch();
            ...
            if ($check) {
                $res_m = $db->query("SELECT `user_id` FROM `forum` WHERE `refid` = '" . $res['refid'] . "' ORDER BY `id` DESC LIMIT 1")->fetch();
...
if (!$error) {
    switch ($do) {
        case 'restore':
            ...
            $req_u = $db->query("SELECT `postforum`, `postthank` FROM `users` WHERE `id` = '" . $res['user_id'] . "'");
            $delthank = $db->query("SELECT * FROM `cms_forum_thank` WHERE `fid` = '$id';")->rowCount();
            if ($req_u->rowCount()) {
                // Добавляем один балл к счетчику постов юзера
                $res_u = $req_u->fetch();
                $db->exec("UPDATE `users` SET `postforum` = '" . ($res_u['postforum'] + 1) . "', `postthank` = '" . ($res_u['postthank'] + $postthank) . "' WHERE `id` = '" . $res['user_id'] . "'");
            }
        ...
        case 'delete':
            ...
            if ($res['close'] != 1) {
                ...
                if ($req_u->rowCount()) {
                    ...
                    $postthank = $res_u['postthank'] > 0 ? $res_u['postthank'] - $delthank : 0;
                    $db->exec("UPDATE `users` SET `postforum` = '" . $postforum . "', `postthank` = '" . $postthank . "' WHERE `id` = '" . $res['user_id'] . "'");
                }
            }

            if ($systemUser->rights == 9 && !isset($_GET['hide'])) {
                // Удаление поста (для Супервизоров)
                $req_f = $db->query("SELECT `filename` FROM `cms_forum_files` WHERE `post` = '$id' LIMIT 1");
                ...
                $db->exec("DELETE FROM `cms_forum_thank` WHERE `fid` = '$id'");
                $db->exec("DELETE FROM `cms_mail` WHERE `sys` = '1' AND `sys_ext` = '" . $id . "'");

                if ($posts < 2) {
                    // Пересылка на удаление всей темы
                    header('Location: index.php?act=deltema&id=' . $res['refid']);
                } else {
                    header('Location: ' . $tools->rewriteUrl($topic['text'], $res['refid'], $page));
                }
            } else {
                // Скрытие поста
                $req_f = $db->query("SELECT * FROM `cms_forum_files` WHERE `post` = '$id' LIMIT 1");
        ...
        default:
            ...
            if (isset($_POST['submit'])) {
                ...
                echo '<textarea rows="' . $systemUser->getConfig()->fieldHeight . '" name="msg" onkeyup="auto_grow(this)">' . (empty($_POST['msg']) ? htmlentities($res['text'], ENT_QUOTES, 'UTF-8') : $tools->checkout($_POST['msg'])) . '</textarea><br>';
