...
if ($systemUser->rights == 3 || $systemUser->rights >= 6) {
    ...
    if (isset($_POST['submit'])) {
        ...
        echo '<form action="index.php?act=addvote&amp;id=' . $id . '" method="post">' .
            ...
            '<input type="text" size="20" maxlength="150" name="name_vote" value="' . (isset($_POST['name_vote']) ? htmlentities($_POST['name_vote'], ENT_QUOTES, 'UTF-8') : '') . '"/><br>';
        ...
        for ($vote = 0; $vote < $_POST['count_vote']; $vote++) {
            echo _t('Answer') . ' ' . ($vote + 1) . '(max. 50): <br><input type="text" name="' . $vote . '" value="' . (isset($_POST[$vote]) ? htmlentities($_POST[$vote], ENT_QUOTES, 'UTF-8') : '') . '"/><br>';
        }
        ...
        echo '<a href="' . $tools->rewriteUrl($resText, $id) . '">' . _t('Back') . '</a>';
    }
} else {
