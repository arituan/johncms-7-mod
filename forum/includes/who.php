...
if ($id) {
    ...
    if ($req->rowCount()) {
        $res = $req->fetch();
        echo '<div class="phdr"><b>' . _t('Who in Topic') . ':</b> <a href="' . $tools->rewriteUrl($res['text'], $id) . '">' . $res['text'] . '</a></div>';
        ...
        if ($total) {
            ...
            for ($i = 0; $res = $req->fetch(); ++$i) {
                ...
                $set_user['avatar'] = 0;
                echo $tools->displayUser($res, 0, ($act == 'guest' || (isset($res['rights']) && $systemUser->rights >= 1 && $systemUser->rights >= $res['rights']) ? 1 : 0));
                echo '</div>';
            }
        } else {
