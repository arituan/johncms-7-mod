...
if ($systemUser->rights == 3 || $systemUser->rights >= 6) {
    ...
    if (isset($_POST['submit'])) {
        ...
        $db->exec("UPDATE `forum` SET
            ...
            WHERE `id` = '$id'
        ");
        header('Location: ' . $tools->rewriteUrl($resText, $id));
    } else {
        // Перенос темы
        $ms = $typ->fetch();
        require('../system/head.php');
        ...
        if (empty($_GET['other'])) {
            $rz1 = $db->query("SELECT `text` FROM `forum` WHERE id='" . $ms['refid'] . "'")->fetch();
            $other = $rz1['refid'];
        } else {
        ...
        echo '<div class="phdr"><a href="index.php?id=' . $id . '"><b>' . _t('Forum') . '</b></a> | ' . _t('Move Topic') . '</div>' .
            ...
            '<p><h3>' . _t('Section') . '</h3>' .
            '<select name="razd">';
        $raz = $db->query("SELECT `id`, `text` FROM `forum` WHERE `refid` = '$other' AND `type` = 'r' AND `id` != '" . $ms['refid'] . "' ORDER BY `realid` ASC");
        ...
        echo '</select></p>' .
            ...
            '</div></form>' .
            '<div class="phdr"><b>' . _t('Other categories') . '</b></div>';
        $frm = $db->query("SELECT `id`, `text` FROM `forum` WHERE `type` = 'f' AND `id` != '$other' ORDER BY `realid` ASC");
