...
if ($systemUser->rights == 3 || $systemUser->rights >= 6) {
    ...
    $ms = $db->query("SELECT `text`, `refid`, `type` FROM `forum` WHERE `id` = '$id'")->fetch();
    ...
    if (isset($_POST['submit'])) {
        $nn = filter_has_var(INPUT_POST, 'nn')
    ? $tools->substring(filter_input(INPUT_POST, 'nn', FILTER_SANITIZE_SPECIAL_CHARS, ['flag' => FILTER_FLAG_ENCODE_HIGH]), 100)
    : '';
        $nn = $container->get('bbcode')->notags($nn);
        ...
        header('Location: ' . $tools->rewriteUrl($ms['text'], $id));
    } else {
