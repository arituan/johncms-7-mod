...
if (!isset($type1) || $type1['type'] != 'm' || $type1['user_id'] != $systemUser->id) {
    echo $tools->displayError(_t('Wrong data'));
    require('../system/end.php');
    exit;
}

$res2 = $db->query("SELECT `refid`, `text` FROM `forum` WHERE `id` = '" . $type1['refid'] . "'")->fetch();
// Проверяем лимит времени, отведенный для выгрузки файла
if ($type1['time'] < (time() - 180)) {
    echo $tools->displayError(_t('The time allotted for the file upload has expired'), '<a href="' . $tools->rewriteUrl($res2['text'], $type1['refid'], $page) . '">' . _t('Back') . '</a>');
    require('../system/end.php');
    exit;
}
...
if (isset($_POST['submit'])) {
    ...
    if ($do_file) {
        ...
        if (!$error) {
            ...
            $res3 = $db->query("SELECT `refid` FROM `forum` WHERE `id` = '" . $res2['refid'] . "'")->fetch();
    ...
    $pa2 = $db->query("SELECT `id` FROM `forum` WHERE `type` = 'm' AND `refid` = '" . $type1['refid'] . "'")->rowCount();
    $page = ceil($pa2 / $kmess);
    echo '<br><a href="' . $tools->rewriteUrl($res2['text'], $type1['refid'], $page) . '">' . _t('Continue') . '</a><br>';
} else {
