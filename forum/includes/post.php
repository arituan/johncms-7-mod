...
// Запрос сообщения
$res = $db->query("SELECT `forum`.*, `users`.`sex`, `users`.`rights`, `users`.`lastdate`, `users`.`status`, `users`.`datereg`, `users`.`rights`
FROM `forum` LEFT JOIN `users` ON `forum`.`user_id` = `users`.`id`
WHERE `forum`.`type` = 'm' AND `forum`.`id` = '$id'" . ($systemUser->rights >= 7 ? "" : " AND `forum`.`close` != '1'") . " LIMIT 1")->fetch();

// Запрос темы
$them = $db->query("SELECT `text` FROM `forum` WHERE `type` = 't' AND `id` = '" . $res['refid'] . "'")->fetch();
...
echo '<div class="phdr"><a href="' . $tools->rewriteUrl($them['text'], $res['refid'], $page) . '">' . _t('Back to topic') . '</a></div>';
