...
switch ($do) {
    ...
    default :
        ...
        $req = $db->query("SELECT `forum`.`user_id`, `forum`.`from`, `users`.`rights`, COUNT(`forum`.`from`) AS `count` FROM `forum` INNER JOIN `users` ON `forum`.`user_id` = `users`.`id` WHERE `forum`.`refid` = '$id' GROUP BY `forum`.`from` ORDER BY `forum`.`from`");
        $total = $req->rowCount();
        ...
        if ($total) {
            ...
            while ($res = $req->fetch()) {
                ...
                echo '<input type="checkbox" name="users[]" value="' . $res['user_id'] . '"/>&#160;' .
                    '<a href="../profile/?user=' . $res['user_id'] . '">' . $tools->displayUsername($res['from'], $res['rights']) . '</a> [' . $res['count'] . ']</div>';
                ++$i;
            }
