...
if ($systemUser->isValid()) {
    ...
    $refUrl = isset($_POST['ref']) ? trim($_POST['ref']) : null;
            ...
            if ($systemUser['failed_login'] > 2) {
                ...
                unset($_SESSION['code']);    
                    ...
                    echo '<form action="login.php' . ($id ? '?id=' . $id : '') . '" method="post">' .
                        ...
                        '<input type="hidden" name="p" value="' . htmlspecialchars($user_pass) . '"/>' .
                        '<input type="hidden" name="mem" value="' . htmlspecialchars($user_mem) . '"/>' .
                        '<input type="hidden" name="ref" value="' . htmlspecialchars($refUrl) . '"/>' .
                        '<input type="submit" name="submit" value="' . _t('Continue', 'system') . '"/></p></div></form>';
                }
            }

            if ($systemUser['failed_login'] < 3 || $captcha) {
                if ($tools->passwordVerify($user_pass, $systemUser['password'])) {
                    // Если логин удачный
                    $display_form = 0;
                    ...
                    if (!$systemUser['preg']) {
                        ...
                        if (isset($_POST['mem'])) {
                            ...
                            $cssid = $tools->generateString();
                            setcookie("cuid", $cuid, time() + 3600 * 24 * 365);
                            setcookie("cussid", $cssid, time() + 3600 * 24 * 365);
                        }

                        // Установка данных сессии
                        $_SESSION['uid'] = $systemUser['id'];
                        $_SESSION['ussid'] = $cssid;

                        $db->exec("UPDATE `users` SET `sestime` = '" . time() . "', `ssid` = '" . $tools->passwordHash($cssid) . "' WHERE `id` = " . $systemUser['id']);
                        ...
                        if ($systemUser['lastdate'] < (time() - 3600) && $set_user['digest']) {
                            ...
                            if ($refUrl)
                                header('Location: ' . strip_tags(base64_decode($refUrl)));
                            else
    ...
    if ($display_form) {
        ...
        echo '<div class="gmenu"><form action="login.php" method="post"><p>' . _t('Username', 'system') . ':<br>' .
            ...
            '<p><input type="hidden" name="ref" value="' . ($refUrl ? $refUrl : htmlentities(base64_encode(getenv("HTTP_REFERER")), ENT_QUOTES, 'UTF-8')) . '"></p>' . 
