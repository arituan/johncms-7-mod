...
if (isset($_POST['submit'])) {
    ...
    unset($_SESSION['code']);
    if (!isset($_POST['token']) || strlen($_POST['token']) < 28 || !$_SESSION['token'] || trim($_POST['token']) != $_SESSION['token'])
        $error['token'] = _t('Wrong data');
    unset($_SESSION['token']);
    // Проверка переменных
    if (empty($error)) {
        $pass = $tools->passwordHash($reg_pass);
...
$token = substr(md5(rand(1000, 1000000)), 0, rand(28, 32));
$_SESSION['token'] = $token;

echo '<form action="index.php" method="post"><div class="gmenu">' .
    '<p><h3>' . _t('Choose Nickname') . ' <b>(<span style="color: red;">*</span>)</b></h3>' .
    ...
    '<p><h3>' . _t('Assign a password') . ' <b>(<span style="color: red;">*</span>)</b></h3>' .
    ...
    '<p><h3>' . _t('Select Gender') . ' <b>(<span style="color: red;">*</span>)</b></h3>' .
    ...
    '</select></p>' .
    '<p>' .
    '<h3>' . _t('Verification code') . ' <b>(<span style="color: red;">*</span>)</b></h3>' .
