...
switch ($act) {
    ...
    case 'say':
        ...
        if (!isset($_POST['token']) || !isset($_SESSION['token']) || trim($_POST['token']) != $_SESSION['token']) {
            $error[] = _t('Wrong data');
        }
    ...
    default:
        ...
        if ($total) {
            if (isset($_SESSION['ga']) && $systemUser->rights >= "1") {
                ...
                echo '<div class="rmenu"><b>Аdmin-Club</b></div>';
                $req = $db->query("SELECT `guest`.*, `guest`.`id` AS `gid`, `users`.`rights`, `users`.`lastdate`, `users`.`sex`, `users`.`status`, `users`.`datereg`, `users`.`id`, `users`.`postforum`
