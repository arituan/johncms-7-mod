...
switch ($mod) {
    case 'change':
        ...
        if (!$error && $user['id'] == $systemUser->id && !$tools->passwordVerify($oldpass, $user['password'])) {
        ...
        if (!$error) {
            // Записываем в базу
            $db->prepare('UPDATE `users` SET `password` = ?, `ssid` = \'\' WHERE `id` = ?')->execute([
                $tools->passwordHash($newpass),
                $user['id'],
            ]);

            // Проверяем и записываем COOKIES
            if (isset($_COOKIE['cuid']) && isset($_COOKIE['cussid'])) {
                setcookie('cussid', '');
