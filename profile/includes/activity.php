...
$menu = [
    ...
    ($mod == 'comments' ? '<b>' . _t('Guestbook') . '</b>' : '<a href="?act=activity&amp;mod=comments&amp;user=' . $user['id'] . '">' . _t('Guestbook') . '</a>'),
];
...
switch ($mod) {
    case 'comments':
        ...
        $req = $db->query("SELECT `text`, `time` FROM `guest` WHERE `user_id` = '" . $user['id'] . "'" . ($systemUser->rights >= 1 ? '' : " AND `adm` = '0'") . " ORDER BY `id` DESC LIMIT $start, $kmess");
    ...
    case 'topic':
        ...
        $req = $db->query("SELECT `id`, `text`, `refid`, `time` FROM `forum` WHERE `user_id` = '" . $user['id'] . "' AND `type` = 't'" . ($systemUser->rights >= 7 ? '' : " AND `close`!='1'") . " ORDER BY `id` DESC LIMIT $start, $kmess");

        if ($req->rowCount()) {
            $i = 0;

            while ($res = $req->fetch()) {
                $post = $db->query("SELECT `text` FROM `forum` WHERE `refid` = '" . $res['id'] . "'" . ($systemUser->rights >= 7 ? '' : " AND `close`!='1'") . " ORDER BY `id` ASC LIMIT 1")->fetch();
                $section = $db->query("SELECT `id`, `text`, `refid` FROM `forum` WHERE `id` = '" . $res['refid'] . "'")->fetch();
                $category = $db->query("SELECT `id`, `text` FROM `forum` WHERE `id` = '" . $section['refid'] . "'")->fetch();
                $text = mb_substr($post['text'], 0, 300);
                $text = $tools->checkout($text, 2, 1);
                echo ($i % 2 ? '<div class="list2">' : '<div class="list1">') .
                    '<a href="' . $tools->rewriteUrl($res['text'], $res['id']) . '">' . $res['text'] . '</a>' .
                    '<br />' . $text . '...' .
                    '<div class="sub">' .
                    '<a href="' . $tools->rewriteUrl($category['text'], $category['id']) . '">' . $category['text'] . '</a> | ' .
                    '<a href="' . $tools->rewriteUrl($section['text'], $section['id']) . '">' . $section['text'] . '</a>' .
    ...
    default:
        ...
        $req = $db->query("SELECT `id`, `text`, `refid`, `time` FROM `forum` WHERE `user_id` = '" . $user['id'] . "' AND `type` = 'm' " . ($systemUser->rights >= 7 ? '' : " AND `close`!='1'") . " ORDER BY `id` DESC LIMIT $start, $kmess");

        if ($req->rowCount()) {
            $i = 0;
            while ($res = $req->fetch()) {
                $topic = $db->query("SELECT `id`, `text`, `refid` FROM `forum` WHERE `id` = '" . $res['refid'] . "'")->fetch();
                $section = $db->query("SELECT `id`, `text`, `refid` FROM `forum` WHERE `id` = '" . $topic['refid'] . "'")->fetch();
                $category = $db->query("SELECT `id`, `text` FROM `forum` WHERE `id` = '" . $section['refid'] . "'")->fetch();
                ...
                echo ($i % 2 ? '<div class="list2">' : '<div class="list1">') .
                    '<a href="' . $tools->rewriteUrl($topic['text'], $topic['id']). '">' . $topic['text'] . '</a>' .
