...
switch ($act) {
    case 'sent':
        ...
        $nick = isset($_POST['nick']) ? $tools->rusLat(trim($_POST['nick'])) : '';
        $email = isset($_POST['email']) ? htmlspecialchars(trim($_POST['email'])) : '';
        $code = isset($_POST['code']) ? trim($_POST['code']) : '';
        $check_code = md5(rand(1000, 99999));
        $error = false;
    ...
    case 'set':
        ...
        if (!$error) {
            ...
            if (mail($res['mail'], $subject, $mail, $adds)) {
                $db->exec("UPDATE `users` SET `rest_code` = '', `password` = " . $db->quote($tools->passwordHash($pass)) . " WHERE `id` = " . $id);
